...
function calculateBreakdown(monthlyCTC, daysWorked = 30, totalDaysInMonth = 30) {
...
async function runJuneForAll() {
  ...
  for (const emp of employees) {
    const b = calculateBreakdown(emp.salary, 30, 30);
    
    const existing = storage.data.paymentRecords.find(r => r.employeeId === emp.id && (r.month === month || r.month === "June 2026"));
  ...
  console.log(`June 2026 Payroll Run Finished: ${updatedCount} updated, ${createdCount} created.`);
}
