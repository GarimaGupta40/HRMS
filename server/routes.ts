...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment-records", async (req, res, next) => {
    try {
      ...
      const allRecords = await storage.getPaymentRecordsByEmployee(empId);
      const normalizeMon = (m: string) => m ? m.trim().substring(0, 3) + " " + m.trim().slice(-4) : "";
      const targetMonNorm = normalizeMon(month);

      const existing = allRecords.find(r => normalizeMon(r.month) === targetMonNorm);

      if (existing) {
        const updated = await storage.updatePaymentRecord(existing.id, req.body);
