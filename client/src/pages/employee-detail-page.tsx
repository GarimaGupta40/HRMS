...
export default function EmployeeDetailPage() {
  ...
  // Calculate salary breakdown using single source of truth
  ...
  const latestPaymentRecord = paymentRecords.length > 0 ? paymentRecords[0] : null;

  const salaryBreakdown = useMemo(() => {
    if (latestPaymentRecord && latestPaymentRecord.grossSalary !== undefined && latestPaymentRecord.grossSalary !== null) {
      return {
        monthlyCTC: employee?.salary || 0,
        grossSalary: latestPaymentRecord.grossSalary,
        basicSalary: latestPaymentRecord.basicSalary ?? 0,
        hra: latestPaymentRecord.hra ?? 0,
        da: latestPaymentRecord.da ?? 0,
        specialAllowance: latestPaymentRecord.specialAllowance ?? 0,
        conveyance: latestPaymentRecord.conveyance ?? 0,
        medical: latestPaymentRecord.medical ?? 0,
        lta: 0,
        child: 0,
        oth: 0,
        bonus: latestPaymentRecord.bonus ?? 0,
        epf: latestPaymentRecord.epf ?? 0,
        esic: latestPaymentRecord.esic ?? 0,
        professionalTax: latestPaymentRecord.professionalTax ?? 0,
        mlwfEmployee: latestPaymentRecord.mlwf ?? 0,
        mlwfEmployer: 0,
        tds: latestPaymentRecord.tds ?? 0,
        totalDeductions: latestPaymentRecord.totalDeductions ?? (latestPaymentRecord.grossSalary - latestPaymentRecord.amount),
        netSalary: latestPaymentRecord.amount,
        daysWorked: latestPaymentRecord.daysWorked ?? 30,
        totalDaysInMonth: latestPaymentRecord.totalDaysInMonth ?? 30
      };
    }
    return getSalaryBreakdown(employee?.salary || 0, 30, 30);
  }, [employee, latestPaymentRecord, currentSalaryComponents]);
    ...
    const monthlyPayments = Object.entries(monthlyGroups)
      ...
      .map(([month, records]) => {
        ...
        const totalGrossAmount = records.reduce((sum, record) => {
          ...
          const breakdown = getSalaryBreakdown(monthlyCTC, daysWorked, totalDaysInMonth);
  ...
  const generatePayslipPDF = (historyItem?: any) => {
    ...
    // Otherwise fall back to latest payment record or current employee salary
    const record = historyItem?.latestRecord || latestPaymentRecord;

    const b = (() => {
      if (record && record.grossSalary !== undefined && record.grossSalary !== null) {
        return {
      ...
      return historyItem ? getSalaryBreakdown(historyItem.monthlyCTC || employee?.salary || 0, 30, 30) : salaryBreakdown;
    })();

    const payrollMonth = record?.month || historyItem?.month || format(new Date(), 'MMMM yyyy');
    ...
    generateProfessionalPayslip({
      ...
      paidDays: paidDays,
      lopDays: Math.max(0, totalDays - paidDays),
    ...
    toast({
      title: "Success",
      description: `Payslip downloaded for ${payrollMonth}`,
    });
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="p-6 bg-emerald-950/10 rounded-3xl border border-emerald-500/20 flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">For {salaryBreakdown.daysWorked || 30} payable days</p>
                  </div>
                  <div className="p-2 bg-white/[0.04] rounded-xl shadow-sm">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 bg-white/[0.02] rounded-xl border border-white/[0.08] flex items-center justify-between">
                      <p className="text-sm text-slate-400">
                        Monthly CTC: ₹{employee.salary?.toLocaleString() || '0'} / {salaryBreakdown.totalDaysInMonth || 30} × {salaryBreakdown.daysWorked || 30} payable days
                      </p>
