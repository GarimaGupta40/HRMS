...
export default function PayrollPage() {
  ...
  const calculateGrossSalary = (monthlyCTC: number, daysWorked: number = 30, totalDaysInMonth: number = 30) =>
    calculatePayrollBreakdown(monthlyCTC, { daysWorked, totalDaysInMonth }).grossSalary;

  // Step 2: Calculate earnings breakdown (using dynamic percentages)
  const calculateBasicSalary = (grossSalary: number) =>
    Math.round((grossSalary || 0) * ((currentSalaryComponents.basicSalaryPercentage || 50) / 100));
  const calculateHRA = (basicSalary: number) =>
    Math.round((basicSalary || 0) * ((currentSalaryComponents.hraPercentage || 50) / 100));

  // Additional earnings components
  const calculateDA = (basicSalary: number) => Math.round((basicSalary || 0) * 0.10);
  const calculateTransAll = (_basicSalary: number, daysWorked: number = 30, totalDaysInMonth: number = 30) =>
    daysWorked !== totalDaysInMonth ? Math.round((1900 / totalDaysInMonth) * daysWorked) : 1900;
  const calculateLTA = (_basicSalary: number) => 0;
  const calculateChildAll = (_basicSalary: number) => 0;
  const calculateMedAll = (_basicSalary: number, daysWorked: number = 30, totalDaysInMonth: number = 30) =>
    daysWorked !== totalDaysInMonth ? Math.round((1250 / totalDaysInMonth) * daysWorked) : 1250;
  const calculateOthAll = (_basicSalary: number) => 0;
  const calculateBonus = (_basicSalary: number) => 0;

  const calculateSpecialAllowance = (grossSalary: number, basicSalary: number, hra: number, da: number, trans: number, _lta: number, _child: number, med: number, _oth: number) => {
    const nonSpecialEarnings = (basicSalary || 0) + (hra || 0) + (da || 0) + (trans || 0) + (med || 0);
    return Math.max(0, (grossSalary || 0) - nonSpecialEarnings);
  };
  ...
  const handleDownloadPayslip = (employee: User) => {
    ...
    const existingRecord = paymentRecords.find(r => r.employeeId === employee.id && r.month === payslipMonth);

    const b = existingRecord && existingRecord.grossSalary ? {
      grossSalary: existingRecord.grossSalary,
      basicSalary: existingRecord.basicSalary ?? 0,
      hra: existingRecord.hra ?? 0,
      da: existingRecord.da ?? 0,
      specialAllowance: existingRecord.specialAllowance ?? 0,
      trans: existingRecord.conveyance ?? 0,
      med: existingRecord.medical ?? 0,
      epf: existingRecord.epf ?? 0,
      esic: existingRecord.esic ?? 0,
      professionalTax: existingRecord.professionalTax ?? 0,
      totalDeductions: existingRecord.totalDeductions ?? 0,
      netSalary: existingRecord.amount ?? 0,
      daysWorked: existingRecord.daysWorked ?? 30,
      totalDaysInMonth: existingRecord.totalDaysInMonth ?? 30
    } : getSalaryBreakdown(employee.salary || 0, existingRecord?.daysWorked ?? 30, existingRecord?.totalDaysInMonth ?? 30);

    const paidDays = b.daysWorked ?? 30;
    const totalDays = b.totalDaysInMonth ?? 30;
  ...
  // Convenience function to get all salary components using single source of truth
  const getSalaryBreakdown = (monthlyCTC: number, daysWorked: number = 30, totalDaysInMonth: number = 30) => {
    const res = calculatePayrollBreakdown(monthlyCTC, {
      daysWorked,
      totalDaysInMonth,
      basicSalaryPercentage: currentSalaryComponents.basicSalaryPercentage,
      hraPercentage: currentSalaryComponents.hraPercentage,
      epfPercentage: currentSalaryComponents.epfPercentage,
      esicPercentage: currentSalaryComponents.esicPercentage,
      professionalTax: currentSalaryComponents.professionalTax,
      mlwfEmployee: calculateMLWFEmployee(),
    });
    return {
      monthlyCTC: res.monthlyCTC,
      daysWorked: res.daysWorked,
      totalDaysInMonth: res.totalDaysInMonth,
      grossSalary: res.grossSalary,
      basicSalary: res.basicSalary,
      hra: res.hra,
      da: res.da,
      trans: res.conveyance,
  ...
  const initializePayrollOverrides = () => {
    ...
    employees.forEach(emp => {
      ...
      let daysWorked: number = totalDays;
      if (empAttendance.length > 0) {
        const absentDays = empAttendance.filter((r: any) => r.status === 'absent').length;
        const halfDays = empAttendance.filter((r: any) => r.status === 'halfday').length;
        const unpaidDays = absentDays + (0.5 * halfDays);
        daysWorked = Math.max(1, totalDays - unpaidDays);
      }
      ...
      try {
        const response = await fetch("/api/payment-records", {
          ...
          body: JSON.stringify({
            ...
            daysWorked: override.daysWorked,
            totalDaysInMonth: override.totalDaysInMonth,
            grossSalary: override.grossSalary,
            basicSalary: override.basicSalary,
            hra: override.hra,
            da: override.da,
            specialAllowance: override.specialAllowance,
            conveyance: override.conveyance,
            medical: override.medical,
            otherAllowance: override.otherAllowance || 0,
            bonus: 0,
            overtime: 0,
            epf: override.epf,
            esic: override.esic,
            professionalTax: override.professionalTax,
            tds: 0,
            mlwf: 0,
            otherDeductions: 0,
            totalDeductions: override.grossSalary - override.netSalary
          }),
          credentials: "include",
        });
  ...
  const handleCompletePayment = () => {
    ...
    const fullBreakdownPayload = {
      daysWorked: paymentForm.daysWorked,
      totalDaysInMonth: paymentForm.totalDaysInMonth,
      grossSalary: Math.round(breakdown.grossSalary),
      basicSalary: Math.round(breakdown.basicSalary),
      hra: Math.round(breakdown.hra),
      da: Math.round(breakdown.da),
      ...
      esic: Math.round(breakdown.esic),
      professionalTax: Math.round(breakdown.professionalTax),
      totalDeductions: Math.round(breakdown.totalDeductions),
  ...
  if (isEmployeePayroll) {
    ...
    // Calculate salary breakdown using the same single source of truth formula
