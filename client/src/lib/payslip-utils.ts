...
export const generateProfessionalPayslip = (data: PayslipData) => {
  ...
  const renderPdf = () => {
    ...
    // Standard Payroll Calculation Logic
    const monthlyGross = b.gross || (b.basic + b.da + b.hra + b.conveyance + b.medical + b.specialAllowance + (b.bonus || 0) + (b.overtime || 0));
    ...
    const loanAdvance = b.loanAdvance || 0;
    const otherDeductions = b.otherDeductions || 0;
    const bonusEarned = b.bonus || 0;
    const overtimeEarned = b.overtime || 0;
    const totalDeductions = b.deductions ?? (epfEmployee + esicEmployee + professionalTax + mlwfEmployee + tdsEmployee + loanAdvance + otherDeductions);
    ...
    const tableData = [
      ...
      ["Medical Allowance", `Rs. ${Math.round(b.medical || 0).toLocaleString()}`, "0.00", "MLWF", `Rs. ${Math.round(mlwfEmployee).toLocaleString()}`, "0.00"],
      ["Special Allowance", `Rs. ${Math.round(b.specialAllowance || 0).toLocaleString()}`, "0.00", "Loan / Advance Recovery", `Rs. ${Math.round(loanAdvance).toLocaleString()}`, "0.00"],
      ["Bonus / Incentives", `Rs. ${Math.round(bonusEarned).toLocaleString()}`, "0.00", "Other Deductions", `Rs. ${Math.round(otherDeductions).toLocaleString()}`, "0.00"],
      ["Overtime (OT) / Arrears", `Rs. ${Math.round(overtimeEarned).toLocaleString()}`, "0.00", "", "", ""],
      ["Gross Earnings", `Rs. ${Math.round(monthlyGross).toLocaleString()}`, "0.00", "Total Deductions", `Rs. ${Math.round(totalDeductions).toLocaleString()}`, "0.00"],
    ];
